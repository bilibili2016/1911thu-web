...
export function setPagesHeight(params) {
  ...
  document.getElementsByClassName('el-main')[0].style.minHeight =
...
返回信息 message 函数
  ...
  msg:提示内容
*/
export function message(vm, types, msg) {
  vm.$message({ showClose: true, type: types, message: msg })
}
...
export function getNet() {
  if (
    process.env.API_STARDUST_BASE_URL.split('.')[0].split('//')[1] ===
    'ceshiapi'
  ) {
    return 'http://ceshi.1911edu.com:2120'
  } else {
    return 'http://api.1911edu.com:2120'
  }
}
