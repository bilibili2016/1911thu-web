...
class Home {
  ...
  // 获取免费课程列表
  async getFreeCourseList({ pages, limits, isFree }) {
  ...
  // 绑定机构id
  async bindingCompanyId({ companyId }) {
  ...
  // hr入口 添加机构信息
  async addCompany({ companyname, person, phones, codes, userID }) {
  ...
  async commitOrder({ ticketId, companyId, types }) {
  ...
  // 获取默认发票信息
  async getInvoiceDetail({ isDefault }) {
    let data = await this.http.get('Wapi/Invoice/invoiceDetail', {
  ...
  // 获取定制消息
  async getPointList() {
    let data = await this.http.post('Wapi/Index/getPointList')
    return data
  }
}
