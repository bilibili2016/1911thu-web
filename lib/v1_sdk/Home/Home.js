...
class Home {
  ...
  // 获取新上课程列表
  getNewCourseList({ pages, limits, evaluateLimit, isevaluate }) {
  ...
  // 提交订单成功后回调 payComplete     见OPayResult.js
  ...
  // 报告问题
  async reportProblem({ content, curriculumId, curriculumcatalogid, types }) {
    let data = await this.http.post('Wapi/Problem/addProblem', {
      problem_content: content,
      curriculum_id: curriculumId,
      curriculum_catalog_id: curriculumcatalogid,
