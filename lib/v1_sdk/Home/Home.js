...
class Home {
  ...
  // 获取经典课程列表
  async getClassicCourseList({ pages, limits, categoryId, sortBy }) {
  ...
  // 增加课程
  async addEvaluate({ ids, evaluatecontent, scores, types }) {
  ...
  // 绑定课程id
  async bindingCurriculumPrivate({ invitationCode }) {
  ...
  // 购物车数量变更
  async changeCartNumber({ numbers }) {
