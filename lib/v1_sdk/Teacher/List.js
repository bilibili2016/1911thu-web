...
class List {
  ...
  async submitBeTeacher({
    ...
    let data = await this.http.get('Wapi/Index/submitBeTeacher', {
      ...
      online_status: courseOnline, //  线上课程制作状态
  ...
  // 验证导师是否可以预约
  async verifyAppointmentTeacher({ tid }) {
    let data = await this.http.get('Wapi/Teacher/verifyBooking', {
      teacher_id: tid
    })
    return data
  }
