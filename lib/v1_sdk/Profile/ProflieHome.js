...
class ProflieHome {
  ...
  // 个人中心 获取订单列表
  async getAllOrderData({ pages, limits, payStatus, startTime, endTime }) {
  ...
  // 个人中心 我的订单 搜索
  // async SearchOrderData({ pages, limits, payStatus, startTime, endTime }) {
  //   let data = await this.http.get('Wapi/Order/orderList', {
  //     page: pages,
  //     limit: limits,
  //     pay_status: payStatus,
  ...
  // }
  //未开发票订单
  async orderNotInvoice({ pages, limits }) {
  ...
  //发票历史
  async tickethistory({ pages, limits }) {
  ...
  // 专属兑换码
  async getCodeList({ pages, limits, ordersn }) {
  ...
  // 兑换码管理 课程码列表
  async getRecordList({ pages, limits, code }) {
    let data = await this.http.get(
  ...
  // 自定制项目
  async customerProjectList({ pages, limits }) {
    let data = await this.http.get(
  ...
  //删除自定制项目
  async deleteCustomerProject({ id }) {
    let data = await this.http.get(
