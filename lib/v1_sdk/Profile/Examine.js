...
class Examine {
	...
	// 去考试权限验证
	async validateExamPrivilege({ vipId, type }) {
	...
	// 开始考试创建试卷
	async createExamRecordQuestion({ vipId, type }) {
	...
	//
	// 保存申请证书数据接口
	async addApplyCertificate({ vipID, needPaper, name, tel, province, city, area, add }) {
	...
	// 开始考试创建试卷
	async examRuleInfo({ vipId, type }) {
		let data = await this.http.get('Wapi/Exam/examRuleInfo', {
			id: vipId,
			type: type
		})
		return data
