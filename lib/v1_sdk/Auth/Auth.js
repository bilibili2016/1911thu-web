...
class Auth {
  ...
  // 注册接口
  async signUp({ phones, ectpwd, codes, companyCodes }) {
  ...
  // 账号密码登录接口
  async signIns({ phonenum, ectpwd, loginTypes }) {
  ...
  // 手机验证码 登录接口
  async signInsmobile({ phones, loginTypes, codes }) {
    let data = await this.http.post('Wapi/Login/loginCheck', {
      user_name: phones,
      loginType: loginTypes,
      phoneSmsCode: codes
  ...
  // 忘记密码
  async forgetPasswordAjax({ phones, ectpwd, code }) {
  ...
  async loginWechat({ phones, codes, openid, companyCodes }) {
