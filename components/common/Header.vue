    ...
    methods: {
        ...
        getShopCartNum() {
            if (persistStore.get("token")) {
                header.shopCartList().then(res => {
                    if (res.status === 100008) {} else {
                        if (res.data) {
                            this.setProductsNum({
                                pn: Number(res.data.curriculumCartList.length) +
        ...
        setUserInfo(res) {
            ...
            if (this.user.isTeacher == 1) {
                this.subPagesData = this.teacherPage
            } else {
                this.subPagesData = this.userPage
        ...
        getUserInfo() {
            ...
            if (persistStore.get("token")) {
                ...
                header.getUserInfo().then(res => {
                    this.isSingleLogin(res);
                    persistStore.set("isTeacher", res.data.userInfo.is_teacher)
                });
