<template>
  ...
  <div class="new" :class="{ bgnone : config.card_type !== 'profile'}">
    <el-row class="center">
      ...
      <!-- 经典课程 card -->
      <v-classic :data="data" :config="config" v-if="config.card_type === 'classic'"></v-classic>
      <!-- 项目card -->
      <v-shortlist :cardData="data" v-if="config.card_type === 'project'"></v-shortlist>
      <!-- 非经典 项目 card -->
      <v-card ref="card" :data="data" :config="config" @selectCid="selectCid" v-else></v-card>
    </el-row>
  </div>
</template>
