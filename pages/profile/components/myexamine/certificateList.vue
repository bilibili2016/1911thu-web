<template>
  <div class="lists">
    <div
      class="item clearfix"
      v-for="(item,index) in examineListData"
      :key="index"
    >
      <div
        class="item_left"
      ...
      <div class="item_right">
        ...
        <div v-if="item.isApplyExam==1">
          <span
            v-if="item.isExamRecord!=0"
            class="btn btn_two"
            @click="viewRecord(item)"
          >考试记录</span>
          <span
            v-if="item.isDoneExamStatus==1"
            class="btn btn_five"
            @click="handleLink('/profile/components/myexamine/viewCertificate?id='+item.id)"
          >查看证书</span>
          <span
            v-if="item.isDoneExamStatus==2"
            class="btn btn_four"
            @click="handleLink('/profile/components/myexamine/reviewing?id='+item.id)"
          >审核中</span>
        </div>
        <!-- 不存在证书（申请 ）-->
        <div v-if="item.isApplyExam==0">
          ...
          <div v-if="item.isExamRecord ==1">
            ...
            <div v-if="item.isDoingExamStatus==1">
              ...
              >考试记录</span>
              <span
                v-if="item.examRecordNum <3 && item.examRecordNum>0"
                class="btn btn_one"
                @click="gotoExamine(item)"
              >参加考试</span>
              <span
                class="btn btn_three"
                @click="handleLink('/profile/components/myexamine/applicantCertificate?id='+item.exam_record_id+'&vipID='+item.id)"
              >申请证书</span>
            </div>
            <!-- 未申请过证书 或者 有考试机会，但考试的都不及格 -->
            <div v-if="item.isDoingExamStatus==0">
              <span
                v-if="item.isExamRecord==1"
                class="btn btn_two"
                @click="viewRecord(item)"
              >考试记录</span>
              <span
                v-if="item.examRecordNum!=0"
                class="btn btn_one"
                @click="gotoExamine(item)"
              >参加考试</span>
            </div>
