    ...
    <div class="curriculumCard">
      <div class="topCard">
        <div class="goodplay" v-if="isShowCover">
          <div class="img-con">
            <img v-if="courseList.is_in_vip" :src="teacherInfo.picture" alt="" class="image courseImg">
            <img v-else :src="courseList.picture" alt="" class="image courseImg">
...
export default {
  ...
  props: ["courseList", "teacherInfo", "privileMsg", "config"],
  computed: {
  ...
  methods: {
    ...
    // 左侧播放按钮事件
    handleImgPlay (item) {
      let top = document.getElementById('playerDetail').offsetTop - 130
      window.scrollTo({
        top: top,
        behavior: "smooth"
      })
